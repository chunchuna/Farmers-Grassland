...
func _generate_character_script(script_path: String, want_collision: bool, want_physics: bool) -> void:
	...
	lines.append("\t# Use existing Model node from the .tscn scene.")
	lines.append("\t# NEVER add_child in editor — it bakes nodes into the parent scene.")
	...
	lines.append("\t\t\treturn")
	lines.append("")
	lines.append("\t# Runtime-only fallback if Model node is missing")
	lines.append("\tif Engine.is_editor_hint():")
	lines.append('\t\tpush_warning("Prefab: Model node not found. Open the prefab .tscn to fix.")')
	lines.append("\t\treturn")
	lines.append("")
	...
	if want_collision or want_physics:
		...
		lines.append("\t\tif Engine.is_editor_hint():")
		lines.append("\t\t\treturn  # Don't create nodes in editor — they leak into parent scene")
		...
		lines.append("\t\t\tvar mi: MeshInstance3D = child as MeshInstance3D")
		lines.append("\t\t\tvar mesh_aabb: AABB = mi.get_aabb()")
		lines.append("\t\t\tvar t: Transform3D = mi.global_transform * Transform3D(Basis(), mesh_aabb.position)")
		lines.append("\t\t\tvar local_pos: Vector3 = global_transform.affine_inverse() * t.origin")
		lines.append("\t\t\tvar local_aabb: AABB = AABB(local_pos, mesh_aabb.size)")
